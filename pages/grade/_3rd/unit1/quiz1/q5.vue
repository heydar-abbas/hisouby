<template>
	<NuxtLayout name="quiz">
		<template #quiz_article>
			...
			<!-- Question Content -->
			<QuizQuestionContent
				question="أعد الألوف, ثم أكتب العدد في صورة مِئات وعَشَراتٍ:"
			>
...
<script setup lang="ts">
	...
	function check(): void {
		if (
			(hundreds.value === "50" && tens.value === "500") ||
			(hundreds.value === "٥٠" && tens.value === "٥٠٠")
		) {
			quiz.value.q5 = 1;
			$quizStore.setPopup("احسنت", true, "/grade/_3rd/unit1/quiz1/q6");
		} else {
			quiz.value.q5 = -1;
	...
	function handelSkip(): void {
		...
		$quizStore.setSkipPopup("/grade/_3rd/unit1/quiz1/q6");
	}
