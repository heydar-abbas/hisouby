...
onMounted(() => {
	avatar.value = userInfo.value?.avatar || "/images/no-image.png";
	name.value = userInfo.value?.name || userInfo.value?.email?.split("@")[0];
	email.value = userInfo.value?.email || "yourEmail@mail";
});
